...
Full-stack Python-Framework, das **FastAPI** (JSON-APIs) und **Flask** (HTML-UI) in einer ASGI-App kombiniert.
...
|---|---|
| API | FastAPI (`/api/...`) |
| UI | Flask + HTMX + Jinja2 (`/`) via a2wsgi |
| Persistenz | SQLite (`SqliteStorage`, direkte SQL-Helpers) |
| Scheduler | APScheduler |
| Verschlüsselung | Fernet (Secrets) |
| Python | ≥ 3.11 |

**Einstiegspunkt für Apps:** `astrapi.core.ui.create(app_root, config, extra_init, modules)`
...
| `ui.py` | Flask-Blueprint (`make_crud_blueprint` + Zusatz-Routen) |
